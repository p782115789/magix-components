...
Magix.applyStyle("mx3e3_mx-form_index_",".mx3e3_mx-form_index_-fail,\n.mx3e3_mx-form_index_-fail:hover {\n  border-color: #a62a22 !important;\n}\n.mx3e3_mx-form_index_-msg {\n  display: none;\n  position: absolute;\n  white-space: nowrap;\n  word-wrap: normal;\n  pointer-events: none;\n  color: #666;\n  font-size: 10px;\n  top: 1px;\n  line-height: 1;\n}\n");
let ByteLen = (str) => {
...
let showError = (beId, key, rule, value) => {
    ...
    node.addClass('mx3e3_mx-form_index_-fail');
    node.each((i, n) => {
        n = $(n);
        let id = n.attr('id');
        if (!id) {
        ...
        if (!msgNode.length) {
            ...
            if (pos == 'static') {
                prt.addClass('mx3e3_mx-style_index_-pr');
            }
            n.after('<div class="mx3e3_mx-form_index_-msg" id="' + msgId + '"/>');
            msgNode = $('#' + msgId);
        }
...
let hideError = beId => {
    let node = $('[mx-beid="' + beId + '"]');
    node.removeClass('mx3e3_mx-form_index_-fail');
    node.each((i, n) => {
        n = $(n);
